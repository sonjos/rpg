...
func enter() -> void:
	...
	if anim_player:
		...
		# Si agilidad_multiplicador es 1.2, la animación va un 20% más rápida
...
func exit() -> void:
	...
	# Restaurar velocidad normal de animaciones al salir del estado
	var anim_player = character.animation_player
	if anim_player:
...
func physics_update(delta: float) -> void:
	...
	# 2. Ventana de combo más rápida (disponible desde el 20% de la animación)
	if pos_anim >= (largo_anim * 0.2):
		can_queue_next = true
		
	# 3. Cancelar recuperación con movimiento pasada el 70% de la animación
	var input_dir = Input.get_vector("move_left", "move_right", "move_forward", "move_back")
	if input_dir != Vector2.ZERO and pos_anim >= (largo_anim * 0.7):
		state_machine.transition_to("Andando")
		return

	# Transición al terminar la animación por completo
...
func _on_hitbox_body_entered(body: Node3D) -> void:
	...
	if nodo_vida and nodo_vida.has_method("recibir_dano"):
		# Calcula el daño usando la fuerza total calculada en PlayerStats
		var dano_ataque = PlayerStats.obtener_fuerza_total() if PlayerStats.has_method("obtener_fuerza_total") else 10.0
		nodo_vida.recibir_dano(dano_ataque)
