...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseMotion:
		...
		# Opcional: Rotación vertical de la cámara (SpringArm)
		var spring_arm = $SpringArm3D
		if spring_arm:
			spring_arm.rotate_x(-event.relative.y * mouse_sensitivity)
			# Limitamos la rotación para que no dé la vuelta completa
			spring_arm.rotation.x = clamp(spring_arm.rotation.x, deg_to_rad(-60), deg_to_rad(60))
	
	# Interacción mediante RayCast
	# Dentro del _unhandled_input en Player.gd:
	if event.is_action_pressed("Interactuar"):
		if raycast and raycast.is_colliding():
			var colisionador = raycast.get_collider()
		
		# Sube por la jerarquía (Hijo -> Padre -> Raíz) hasta encontrar el script
...
func _input(event: InputEvent) -> void:
	# Si presionas ESC, liberas el ratón (útil para salir del juego o entrar a menús)
	if event.is_action_pressed("ui_cancel"):
		Input.mouse_mode = Input.MOUSE_MODE_VISIBLE
		
# Función que llamará el enemigo cuando te golpee
func recibir_dano_jugador(cantidad: float) -> void:
	...
	var health_state_machine = get_node_or_null("HealthStateController") # Cambia por la ruta exacta si tu nodo se llama diferente
